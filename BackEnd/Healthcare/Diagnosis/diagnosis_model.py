...
# model = joblib.load(r'/app/aiModel/diagnosis_model.pkl')
# label_encoder = joblib.load(r'/app/aiModel/label_encoder.pkl')
model = joblib.load(r'./aiModel/diagnosis_model.pkl')
label_encoder = joblib.load(r'./aiModel/label_encoder.pkl')




def make_diagnosis(symptoms):
    # Assuming symptoms is a list of feature names, e.g., ['fever', 'headache', ...]

    # Create a vector of 377 zeros (for 377 symptoms/features)
    feature_vector = np.zeros(377)

    # List of all possible symptoms (features). This should match the model's feature order.
    all_symptoms = [
        ...
        'emotional symptoms','elbow weakness','back weakness','pus in sputum',
        'symptoms of the scrotum and testes','swelling of scrotum','pain in testicles',
        'flatulence','pus draining from ear','jaundice','mass in scrotum','white discharge from eye',
        ...
        'burning chest pain','back cramps or spasms','stiffness all over','muscle cramps', 
        'contractures or spasms','low back cramps or spasms',
        ...
        'sweating','mass on eyelid','swollen eye','eyelid swelling','eyelid lesion or rash','unwanted hair',
        'symptoms of bladder','irregular appearing nails','itching of skin','hurts to breath','nailbiting',
        'skin dryness, peeling, scaliness, or roughness','skin on arm or hand looks infected','skin irritation',
        'itchy scalp','hip swelling','incontinence of stool','foot or toe cramps or spasms','warts',
        ...
        'shoulder lump or mass','polyuria','cloudy eye','hysterical behavior','arm lump or mass','nightmares',
    ...
    # Loop through the provided symptoms and set the respective positions in the feature_vector to 1
    for symptom in symptoms:
        if symptom in all_symptoms:
            index = all_symptoms.index(symptom)  # Find the index of the symptom
            feature_vector[index] = 1  # Set the corresponding index to 1
    
    # Convert the feature_vector into a numpy array to make it compatible with the model
    input_data = np.array([feature_vector])
   
    # Predict the encoded diagnosis
    prediction = model.predict(input_data)[0]

    # Predict probabilities for confidence
    probabilities = model.predict_proba(input_data)
    confidence = np.max(probabilities) * 100  # Get the highest confidence percentage

    # Decode the diagnosis label
    diagnosis = label_encoder.inverse_transform([prediction])[0]

    print("The predicted disease is:", diagnosis, "with confidence:", confidence)
